...
export default function Shifts() {
  ...
  const submit = async (e) => {
    ...
    setSaving(true);
    try {
      if (editing) await api.patch(`/shifts/${editing._id}`, form);
  ...
  const remove = async (item) => {
    if (!confirm(`Delete system shift "${item.name}"?`)) return;
    await api.delete(`/shifts/${item._id}`);
    load();
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
           <h1 className="text-2xl font-bold">System Work Shifts</h1>
           <p className="text-slate-500 text-sm">Manage work hours for platform administrators and system employees.</p>
        </div>
        <Button onClick={() => { setEditing(null); setForm(emptyForm); setModal(true); }} className="w-full sm:w-auto">
          <Plus className="h-4 w-4 mr-2" /> New System Shift
        </Button>
      </div>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editing ? `Edit ${editing.name}` : 'New System Shift'} wide>
